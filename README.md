...
OpenCode TUI 插件 — 在侧边栏显示 API 余额。

## 功能

- **跨平台**：支持 macOS / Linux / Windows 的 opencode TUI 环境
...
```jsonc
{
  "balances": [
    {
      "provider": "DeepSeek",
      "script": "({ request: { url: 'https://api.deepseek.com/user/balance', method: 'GET', headers: { 'Authorization': 'Bearer ${DEEPSEEK_API_KEY}', 'User-Agent': 'cc-switch/1.0' } }, extractor: function(r) { if (!r || r.is_available !== true) return 'N/A'; const i = r.balance_infos && r.balance_infos[0]; if (!i) return 'N/A'; return '剩余: ' + parseFloat(i.total_balance).toFixed(2) + ' ' + (i.currency || 'CNY'); } })"
    }
  ]
}
```
